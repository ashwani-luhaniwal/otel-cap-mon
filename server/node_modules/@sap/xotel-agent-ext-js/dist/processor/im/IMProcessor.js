...
class IMProcessor {
    ...
    onStart(span, parentContext) {
        ...
        const tenantId = parentContext.getValue(api.createContextKey(constants.CALM_ATT_TENANT_ID));
        const serviceType = process.env.SAP_CALM_SERVICE_TYPE || "";
        if (span.kind.valueOf() == api_1.SpanKind.PRODUCER && (span.name === constants.UseCase.IMPUSH)) {
            if (typeof tenantId !== "string" || !NotificationPoller_1.NotificationPoller.isDataCollectionTurnedOn(serviceType, tenantId, span.name)) {
                logger_1.logger.debug(`IMProcessor onStart: data collection is switched off for ${serviceType}-${tenantId}-${span.name}, don't process span`);
                return;
            }
            logger_1.logger.debug(`IMProcessor onStart: data collection is switched ON for ${serviceType}-${tenantId}-${span.name}, processing span...`);
            this.setAttributes(span, parentContext);
            if (parentContext != undefined)
                api_1.trace.setSpan(parentContext, span);
        }
        else {
            logger_1.logger.debug(`IMProcessor onStart: Ignoring kind: ${kind.valueOf()} span: ${span.name}`);
        }
    }
    onEnd(span) {
        ...
        const tenantId = span.attributes[constants.CALM_ATT_TENANT_ID];
        const serviceType = process.env.SAP_CALM_SERVICE_TYPE || "";
        const kind = span.kind;
        if (kind.valueOf() === api_1.SpanKind.PRODUCER && (span.name === constants.UseCase.IMPUSH)) {
            if (typeof tenantId !== "string" || !NotificationPoller_1.NotificationPoller.isDataCollectionTurnedOn(serviceType, tenantId, span.name)) {
                logger_1.logger.debug(`IMProcessor onEnd: data collection is switched off for ${serviceType}-${tenantId}-${span.name}, don't process span`);
                return;
            }
            logger_1.logger.debug(`IMProcessor onEnd: data collection is switched ON for ${serviceType}-${tenantId}-${span.name}, processing span...`);
            this.doExport([span]);
        }
        else {
            logger_1.logger.debug(`IMProcessor onEnd: Ignoring kind: ${kind.valueOf()} span: ${span.name}`);
        }
    }
    shutdown() {
        ...
        api_1.context.with((0, core_1.suppressTracing)(api_1.context.active()), () => {
            this.spanExporter.export(spans, result => {
                ...
                if (result.code !== core_1.ExportResultCode.SUCCESS) {
                    (0, core_1.globalErrorHandler)((_a = result.error) !== null && _a !== void 0 ? _a : new Error(`IMProcessor: failed to export. Status: ${result})`));
                }
            });
        });
    }
    setAttributes(span, context) {
        ...
        span.setAttribute("Processor", "IMProcessor");
...
exports.IMProcessor = IMProcessor;
